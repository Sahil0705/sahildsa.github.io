            ...
            for($n_e=0; $n_e<$edges; $n_e++)
            {
                if($dist[$l[$n_e][0]] != INF && $dist[$l[$n_e][0]] + $l[$n_e][2] < $dist[$l[$n_e][1]])
                {
                    echo "<script>alert('Sorry!!! Graph contains negative weight cycle');</script><br><br><br>
                        <center><a href='b.php'><input type='button' style='width: 120px; height: 50px; background-color: #05a3ff; border: none; font-size: 17px; color: white;' value='Back'></a></center>";
                    return;
                }
            }
            ?>
